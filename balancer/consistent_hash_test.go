...
func TestConsistent_UnregisterHost(t *testing.T) {
	...
	c.Remove("127.0.0.1:8848")
	if len(c.Hosts()) != 1 {
		t.Errorf("Expected 1 node in ring, got %d", len(c.Hosts()))
	}

	c.Remove("127.0.0.1:9999")
	if len(c.Hosts()) != 0 {
...
func TestConsistent_GetKey(t *testing.T) {
	...
	c.Add("192.168.0.1:8999")
	host, err = c.Balance("23452345")
	if err != nil {
		t.Fatal(err)
	}
